...
SBRPG = SBRPG or {}
local defaults={dangerscreening="0",adaptiveordering="0",nodechestloot="0",opportunisticgathers="0",enable="1",debug="0",returnhome="1",minchance="1.0",bagreserve="0",attempts="3",failedblacklist="120",emptyblacklist="120",zone="1",settledelay="750",actiondelay="500",loottargettimeout="5000",gatherclaimtimeout="5000",routeprogresscheck="2500",chestloottimeout="5000",hotspotdwell="15000",emptyhotspotcooldown="120000",dangercooldown="120000",poolrediscovery="60000",bobbers="35591",uselures="1",prioritizepools="0",openwateronly="1",searchdistance="500.0",castdistance="12.0"}
local definitions={
 {section="Experimental farming"},
 {key="dangerscreening",label="Danger screening (beta)",kind="bool",capability="DANGER_SCREENING",tip="Opt-in preemptive danger screening. Ordinary gray packs are exempt, elites are not. May pause unsafe approaches and returns."},
 {key="adaptiveordering",label="Adaptive ordering (beta)",kind="bool",capability="ADAPTIVE_ORDERING",tip="Opt-in node and material-hotspot ordering using bounded session evidence, route outcomes, and requested-item yield."},
 {key="nodechestloot",label="Collect eligible chests",kind="bool",capability="NODE_CHEST_LOOT",tip="Opt-in for node-farming runs: collect nearby chests that pass normal stock-loot checks. Mining/herb selection remains exact; this does not enable broad node scanning."},
 {key="opportunisticgathers",label="Opportunistic gathers",kind="bool",tip="Opt-in: while travelling, gather one eligible live herb or vein within 40 yards, then resume the original node, material hotspot, or fishing destination. Independent of danger/adaptive settings."},
 {section="General"},{key="enable",label="Module enabled",kind="bool",tip="Enable mod-selfbot-rpg."},{key="returnhome",label="Return home on Stop",kind="bool",tip="When enabled, Stop finishes active combat/loot and returns to the session start. Disable for an immediate halt."},{key="debug",label="Debug logging",kind="bool",tip="Show additional server diagnostics and retain DEBUG History entries."},{key="zone",label="Stay in starting zone",kind="bool",tip="Restrict local farming routes to the starting zone."},
 {section="Activity timing"},{key="actiondelay",label="Action delay",kind="number",min=100,max=10000,unit="ms",tip="Minimum delay between SBRPG controller decisions; stock playerbot timing is unchanged."},{key="settledelay",label="Gather settle delay",kind="number",min=0,max=10000,unit="ms",tip="Wait after stock gathering claims a node before reevaluating handoff."},{key="loottargettimeout",label="Loot target timeout",kind="number",min=1000,max=15000,unit="ms",tip="Maximum wait for selected node/corpse loot before releasing a stale stock target."},{key="gatherclaimtimeout",label="Gather claim timeout",kind="number",min=1000,max=15000,unit="ms",tip="Maximum wait for stock gathering to claim a selected node before retrying it."},{key="routeprogresscheck",label="Route progress check",kind="number",min=1000,max=15000,unit="ms",tip="How often SBRPG checks travel progress before safe-route recovery."},{key="chestloottimeout",label="Chest loot timeout",kind="number",min=1000,max=15000,unit="ms",tip="Maximum wait for optional chest loot before resuming the selected node route."},{key="hotspotdwell",label="Hotspot search dwell",kind="number",min=1000,max=600000,unit="ms",tip="Time spent scanning a reached material hotspot before moving on."},{key="emptyhotspotcooldown",label="Empty hotspot cooldown",kind="number",min=1000,max=600000,unit="ms",tip="How long a confirmed empty material hotspot stays out of adaptive selection."},{key="dangercooldown",label="Danger cooldown",kind="number",min=1000,max=600000,unit="ms",tip="How long an unsafe node, hotspot, or corridor remains temporarily excluded."},{key="poolrediscovery",label="Pool rediscovery",kind="number",min=1000,max=600000,unit="ms",tip="Delay before absent or unreachable fishing pools become eligible again."},
...
local function Supported(key)
 for _,def in ipairs(definitions) do
  if def.key==key then return not def.capability or SBRPG.HasCapability(def.capability) end
 end
 return false
end
...
function SBRPG.ApplySavedSettings()
 ...
 for _,def in ipairs(definitions)do if def.key and Supported(def.key) then SBRPG.Send("SET_CONFIG",{def.key,tostring(SelfBotRPGDB.Settings[def.key]or defaults[def.key])})end end
end
SBRPG.RegisterTab("settings","Settings",4,function(panel)
 ...
 function panel:OnCapabilitiesUpdated()
  ...
  for _,def in ipairs(definitions)do if def.capability then
   local enabled=SBRPG.State.bridgeReady and SBRPG.HasCapability("SET_CONFIG") and Supported(def.key)
   SBRPG.SetEnabled(controls[def.key],enabled,"Server does not advertise "..def.capability..".")
  end end
 end
